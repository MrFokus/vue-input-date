<template>
  <div ref="refInputDate" @focus="changeParentFocus" tabindex="0" class="mrfokus-input-date">
    <template v-for="(field, index) in fields" :key="field">
      <input @paste="onPaste" autocomplete="off" class="field" type="text" :name="field"
        :placeholder="value[field]?.placeholder" :value="value[field]?.value"
        :style="`width: ${Number(value[field]?.length) + 1}ch`"
        @input="value[field] ? value[field].changeInput($event, field, value[field]?.length) : () => { }"
        @keydown.backspace="onBackspace($event, field)" @focus="selectField" @keydown.right="focusNext($event, field)"
        @keydown.left="focusPrev($event, field)" @click.stop="selectField" />
      <slot name="divider" v-if="index !== fields.length - 1">
        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12" fill="none">
          <path d="M4.00003 11L9.00003 1" stroke="#D0D5DD" stroke-linecap="round" stroke-linejoin="round" />
        </svg>
      </slot>
    </template>
  </div>
...
const defaultParams: Record<string, { placeholder: string, length: number, format: 'number' | 'string', changeInput: Function }> = {
  dd: { placeholder: 'ДД', length: 2, format: 'number', changeInput: onInputDay },
  mm: { placeholder: 'ММ', length: 2, format: 'number', changeInput: onInputMonth },
  yyyy: { placeholder: 'ГГГГ', length: 4, format: 'number', changeInput: onInputYear },
}
...
onMounted(() => {
  inputs.value = Array.from(refInputDate.value?.querySelectorAll('input.field') ?? [])
})

watch(() => props.modelValue, () => {
  if (props.modelValue)
    fromDate(props.modelValue)
...
function onInputDay(ev: InputEvent, name: string, length: number) {
  ...
  if (value.value[name])
    value.value[name].value = target.value
  onInputFix()
...
function maskNumber(ev: Event, name: string) {
  ...
  if (current?.format === 'number' && (isNaN(Number(val)) || val.toLowerCase().includes('e'))) {
...
function onPaste(ev: ClipboardEvent) {
  ev.preventDefault()
  const text = ev.clipboardData?.getData('text')?.trim().replace(/[\s\-\/.]+/g, '') ?? ''
  let i = 0
...
function toDate() {
  ...
  if (value?.value && value.value?.yyyy?.value && value.value?.mm?.value && value.value?.dd?.value) {
    date.setFullYear(+value.value.yyyy.value, +value.value.mm.value - 1, +value.value.dd.value)
    return date
  }
  return null
}
function fromDate(date: Date) {
  ...
  if (value.value.dd && value.value.mm && value.value.yyyy) {
    ...
    value.value.yyyy.value = String(yyyy < 0 ? 0 : yyyy).padStart(value.value.yyyy.length, '0');
  }
